...
- **KSP `2.3.10` — held** (pinned via a `.github/dependabot.yml` ignore scoped to `com.google.devtools.ksp* == 2.3.10`). KSP 2.3.10 added an `onlyIf` gate that SKIPs `kspDebug<Variant>AndroidTestKotlin` for library-module `androidTest` source sets, so Hilt test-component codegen never runs — `:feature-example` instrumented tests then fail at runtime with `ClassNotFoundException: …ExampleHiltTest_TestComponentDataSupplier` (#217, and the KSP slice of the gradle-and-plugins group #219). Reproduced locally on 2026-07-17: 2.3.9 runs the KSP androidTest task and emits the Hilt injector/aggregated-deps sources; 2.3.10 skips it (`onlyIf 'Task satisfies onlyIf spec' is false`) and emits nothing. Filed upstream as **google/ksp#3050** (linked by the maintainers to the #3046 fix). `2.3.11-SNAPSHOT` worked on 2026-07-21, but the **released `2.3.11` regressed** (verified 2026-08-07, #244/#246): the androidTest task now runs instead of skipping, but KSP emits **no** output for the `androidTest` source set (the `build/generated/ksp/debugAndroidTest` dir is never created), so the supplier still isn't generated and instrumented tests fail the same way. Both `2.3.10` and `2.3.11` are pinned in `.github/dependabot.yml`; the list names known-broken versions only, so Dependabot still surfaces the next release (2.3.12+). Drop the matching pin once a KSP release keeps `instrumented_tests` green.
