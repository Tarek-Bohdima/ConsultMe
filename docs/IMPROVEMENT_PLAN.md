...
- **Hilt 2.59.2 → 2.60** (#200, merged 2026-06-26). Routine patch bump, landed on its own — **does not** unblock Kotlin 2.4.0 (2.60 only bumps Dagger's own Kotlin to 2.3.21, so its `kotlin-metadata-jvm` still caps at metadata 2.3.0). 2.60's generated `ActivityRetainedComponentBuilder.savedStateHandleHolder(...)` is annotated `@CanIgnoreReturnValue` from `com.google.errorprone.annotations`, a `compileOnly` annotation absent from the runtime classpath; R8 full-mode reported it as a missing class and failed `:app:minifyReleaseWithR8`. Fix: added `-dontwarn com.google.errorprone.annotations.**` to `app/proguard-rules.pro` (R8's own `missing_rules.txt` suggested the same `-dontwarn`; broadened to the package since Dagger references several such compile-only annotations). Adopters inherit the rule; no per-fork action needed.
- **Kotlin 2.4.0 bump — deferred, blocked on Hilt + KSP** (tracked in issue #185). Dependabot's Kotlin `2.3.21 → 2.4.0` bumps (#184, #189, #198) fail at `:app:hiltJavaCompileDebug` with `[Hilt] Provided Metadata instance has version 2.4.0, while maximum supported version is 2.3.0` — Hilt's bundled `kotlin-metadata-jvm` is not yet Kotlin-2.4-aware (re-confirmed 2026-06-26: even Hilt 2.60 still bundles Kotlin 2.3.21). Also blocked on KSP, whose latest release is `2.3.9` — no `2.4.0-x` build exists yet, and KSP must match the Kotlin version. Lands as a coordinated `kotlin` + `ksp` + `hilt` bump on the `v4.x-rc` pre-release channel once **both** KSP ships `2.4.0-x` and a Hilt release bumps its bundled Kotlin to 2.4. Same shape as the Phase 9 deferral.
