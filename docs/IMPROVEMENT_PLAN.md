...
- **Dependency maintenance**: AGP 9.4.1, KSP 2.3.12, Hilt 2.60.1, Kover 0.9.9, Spotless 8.10.2, Roborazzi 1.74.0, Robolectric 4.17, Gradle 9.7.1, compose BOM 2026.09.00, Room 2.8.5, kotlinx-serialization-core 1.11.0, androidx baselineprofile/benchmark 1.5.0, plus the testing and github-actions Dependabot groups.
...
- **KSP 2.3.11 unblocked** (#265): the `2.3.10`/`2.3.11` androidTest breakage was a config gap on our side, not a KSP bug. KSP 2.3.10 defaults `ksp.allow.all.target.configuration` to false, so the global `ksp(...)` no longer propagates to test variants and Hilt's `_TestComponentDataSupplier` was never generated. Fixed by declaring `kspAndroidTest`/`kspTest` in `consultme.android.hilt` and bumping KSP `2.3.9 → 2.3.11`; the Dependabot ignore was lifted. Confirmed by the maintainer on google/ksp#3050.
- **Gradle daemon JVM pinned to JDK 21** (#269): `gradle/gradle-daemon-jvm.properties` (Daemon JVM criteria) so local runs stop spawning redundant daemons under mismatched JDKs. The daemon JVM is separate from the `jvmToolchain(17)` compile toolchain, which stays on 17. CI `setup-java` installs both 17 and 21; foojay-resolver covers auto-provisioning elsewhere.
