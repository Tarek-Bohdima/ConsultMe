...
- **Toolchain**: `jvmToolchain(17)`, JVM target 17, `freeCompilerArgs = ["-Xcontext-parameters"]` — set by the convention plugins. The Gradle *daemon* itself runs on JDK 21 (`gradle/gradle-daemon-jvm.properties`, Daemon JVM criteria); that is the JVM Gradle runs on, separate from the JDK 17 compile toolchain. CI installs both (setup-java 17 + 21); locally the daemon auto-provisions 21 via the file's foojay URLs if the launcher JDK differs.
